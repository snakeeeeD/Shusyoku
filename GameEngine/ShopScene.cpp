...
void ShopScene::GenerateStock()
{
    ...
    auto inLayer = [&](int L) { return L == 0 || L == layer; };   // 0=全層
    ...
    for (auto& kv : MaterialDataBase::AllMaterials())
        if (kv.second.buyPrice > 0 && inLayer(kv.second.layer)) matIds.push_back(kv.first);   // 価格0/層外は店に出さない
...
void ShopScene::Draw()
{
    ...
    for (int i = 0; i < (int)m_items.size(); i++)
    {
        ...
        else
        {
            ...
            m_spriteRenderer->DrawSprite(m_whiteTexture, x + 6, y + 6, w - 12, h - 12, 0.0f,
                XMFLOAT4(col.x, col.y, col.z, 0.85f));   // 種類色を中に
        }
    }
    ...
    if (m_removedThisShop)
        m_textRenderer->DrawText(L"削除済み", 95.0f, rmYY + 12.0f, 20.0f, D2D1::ColorF(0.6f, 0.6f, 0.6f));
    else
    {
