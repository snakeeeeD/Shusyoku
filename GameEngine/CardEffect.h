...
namespace CardEffect
{
    ...
    inline void ApplyOnHitEffect(const CardEffectData& effect, BuffManager& target)
    {
        ...
        if (bt == BuffType::Poison) buff.value += RelicManager::SumValue("poisonAdd");   // 毒の心得
    ...
    inline void ApplyEffectToPlayer(const CardEffectData& effect, Player* player)
    {
        ...
        if (effect.type == CardEffectType::ApplyBuff)
        {
            ...
            {
                buff.name = L"攻撃力UP(今ターン)";
                buff.description = L"このターン攻撃力+" + std::to_wstring(effect.value);
            }
            else if (effect.buffType == "AttackGrowth")
            {
                buff.name = L"闘気";
                buff.description = L"毎ターン攻撃力+" + std::to_wstring(effect.value);
            }
